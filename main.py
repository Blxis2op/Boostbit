...
client_identifiers = ['safari_ios_16_0', 'safari_ios_15_6', 'safari_ios_15_5', 'safari_16_0',
...
class logger():
    ...
    def success(self, msg: str):
        print(
            f"{Fore.WHITE}[{Fore.CYAN}{self.times}{Fore.RESET}]({Fore.MAGENTA}*{Fore.RESET}) {Fore.LIGHTGREEN_EX}{msg}{Fore.RESET}")

    def error(self, msg: str):
        print(
            f"{Fore.WHITE}[{Fore.CYAN}{self.times}{Fore.RESET}]({Fore.MAGENTA}*{Fore.RESET}) {Fore.LIGHTRED_EX}{msg}{Fore.RESET}")
...
class Utils():
    ...
    async def isWhitelisted(ctx) -> bool:
        if (
            str(ctx.author.id) in open(
                "whitelist.txt", "r").read().splitlines()
...
bot = discord.Bot(command_prefix="*", activity=activity,
...
async def restock(ctx, code: discord.Option(str, "Paste.ee link", required=True), type: discord.Option(int, "Type of tokens you are restocking, 3 months or 1 month", required=True)):
    ...
    if type != 1 and type != 3 and type != 0:
        return await ctx.respond(embed=discord.Embed(title="**Invalid Input**", description="Type can either be 3 (months), 2 (months) or 1 (Months)", color=discord.Colour.red()))
    if type == 1:
        file = "input/1m_tokens.txt"
    elif type == 2:
        file = "input/2m_tokens.txt"
    elif type == 3:
...
async def boost(
    ...
    months: discord.Option(int, "Uses 1m or 2m or 3m stock to boost servers", required=True
                           ),
